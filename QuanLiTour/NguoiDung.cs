...
namespace QuanLiTour
{
    class NguoiDung
    {
        ...
        public List<string> getDatatable(string TenDangNhap)
        {
            ...
            try
            {
                SqlDataAdapter da = new SqlDataAdapter("select MaManHinh from QL_PhanQuyen, QL_NhomNguoiDung where QL_PhanQuyen.MaNhomNguoiDung = (select MaNhomNguoiDung from QL_NguoiDung, QL_NguoiDungNhomNguoiDung where QL_NguoiDung.TenDangNhap = '" + TenDangNhap + "' and QL_NguoiDung.TenDangNhap = QL_NguoiDungNhomNguoiDung.TenDangNhap) and QL_PhanQuyen.MaNhomNguoiDung = QL_NhomNguoiDung.MaNhom", Properties.Settings.Default.TravelConnectionString);
                da.Fill(dt);
                foreach (System.Data.DataRow row in dt.Rows)
                {
                    foreach (System.Data.DataColumn col in dt.Columns)
